...
?>    
            <div class="box-content">
			 ...
			 $category_id=base64_decode($_GET['category_id']);
			  $db1 	= 	new Database();
			$db1->query('select* from colleges where id=:category_id');
			$db1->bind(':category_id',$category_id);
			$rows = $db1->resultset();
			foreach($rows as $result)
			{
	
		$category=$result['college'];
		$stat=$result['status'];
	...
	<input type="hidden" name="exam_id" id="category_id" value="<?php echo $category_id; ?>" />
                   <div class="form-group">
                        <label>Category</label>
                        <input type="text" id="txtCategory" name="txtcollege" required="required" class="form-control" value="<?php echo $category; ?>">
                    </div>
                     ...
                     <div class="form-group">
                       ...
                       <select name="txtstatus" required="required" class="form-control">
                           ...
                           <option value="1" <?php if($stat=='1'){ echo 'selected';} ?>>
                               Enable
                           </option>
                           <option value="0" <?php if($stat=='0'){ echo 'selected';} ?> >
                               Disable
                           </option>
                       </select>
                    </div>
