<?php include("../../db_connect_inner_code.php");
 ...
 $flcsrf=1;
	...
	if($action=="Add"  && $flcsrf==1)
	{
		...
		$db9 	= 	new Database();
    $db9->insertdata('video_category',array("category" => $category,"exam" => $exam));
	}
	...
	$db1 	= 	new Database();
		$db1->updatedata('video_category',array("category" => $category,"exam" => $exam),array("category_id" => $category_id));
		$db1->dbclose();
	}
	//---------------------------------Delete------------------------------------------------------------------------------
