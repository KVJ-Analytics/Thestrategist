<?php include("../../db_connect_inner_code.php");
	...
	$uname=filter_var($_POST['txtusername'], FILTER_SANITIZE_STRING);
 ...
 /* ------end of csrf protection------------*/
	...
	$oldpass=$oldpass;
	 							$db1 	= 	new Database();
								$stmt=$db1->query('select * from website_login where uname=:uname and pwd=:oldp and exam=:examp');	
	...
	if($db1->rowCount()>0)
	{
		$newpass=$newpass;
	$db2=new Database();
	$db2->updatedata('website_login',array("pwd" =>$newpass),array("uname" =>$uname));
	$db2->dbclose();
	$fl=2;
	}
