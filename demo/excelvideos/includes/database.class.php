<?php // Define configuration
	...
	define("DB_USER", "thestr_excel");
	define("DB_PASS", "X8n]vDxeKkf%");
	define("DB_NAME", "thestr_excel");
		class Database
	{
...
public function updatedata($tablename,$toUpdate = array(), $cid){
	...
	/*	public function insertdata_getid($tablename,$other = array())
	{
		if(count($other) > 0)
		{
			$keys 	= array_keys($other);
 ...
 public function deletedata($tablename,$toUpdate = array()){
		...
		if( is_array($toUpdate) && !isset($toUpdate['c_id'])){
			...
			$columns = substr($columns, 0, -5); // Remove last ","
