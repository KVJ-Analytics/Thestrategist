...
<div class="container-fluid excelhead">
	...
	$k=0;
	$db1 	= 	new Database();
								$stmt=$db1->query('select * from video_category order by category_id asc');	
								$rows = $db1->resultset();
	...
	foreach($rows as $result)
	{
		...
		$category_id=$result['category_id'];
					...
					<div class="tab-pane <?php if($category_id==1){?> active <?php } ?>" id="tab<?php echo $category_id; ?>">                
						<div class="">
                       
                        <?php
                        $stmt2=$db2->query('select * from video_gallery where category_id=:category_id');
	...
	?>
                        <div class="col-sm-6">
                            <video width="100%" controls oncontextmenu="return false;" controlsList="nodownload" data-id="<?php echo $vid;?>">
  <source data-id="<?php echo $vid;?>" src="<?php echo $path;?>" type="video/mp4">
</video>
