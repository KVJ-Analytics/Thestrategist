...
<div class="container-fluid excelhead">
	...
	$k=0;
	$db1 	= 	new Database();
								$stmt=$db1->query('select * from video_category where exam=2 order by category_id asc');	
								$rows = $db1->resultset();
	...
	foreach($rows as $result)
	{
		...
		$category_id=$result['category_id'];
					...
					<div class="tab-pane <?php if($category_id==1){?> active <?php } ?>" id="tab<?php echo $category_id; ?>">                
                        ...
                        <?php
                        $stmt2=$db2->query('select * from video_gallery where exam=2 and category_id=:category_id');
